...
"$repo_root/oracle/.venv/bin/python" "$repo_root/oracle/m1_server_smoke.py" \
    --repo-root "$repo_root" \
...
jq -e '.schema == "hyperion.m1-server-smoke.v1" and .deterministic == true and .repeat_count == 2 and .quality_floor == false' \
    "$output_dir/12b.server-smoke.json" "$output_dir/e4b.server-smoke.json" >/dev/null
echo "m1-server-smoke-pass: $output_dir"
