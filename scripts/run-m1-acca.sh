...
"$binary" m1 run-cell --model 12b --context 4096 --arm acca-a1 \
    --wired-limit default --run-manifest "$run_manifest" --output "$run_root/01-a1.jsonl"
"$binary" m1 run-cell --model 12b --context 4096 --arm acca-c1 \
    --wired-limit "$candidate_bytes" --run-manifest "$run_manifest" --output "$run_root/02-c1.jsonl"
"$binary" m1 run-cell --model 12b --context 4096 --arm acca-c2 \
    --wired-limit "$candidate_bytes" --run-manifest "$run_manifest" --output "$run_root/03-c2.jsonl"
"$binary" m1 run-cell --model 12b --context 4096 --arm acca-a2 \
    --wired-limit default --run-manifest "$run_manifest" --output "$run_root/04-a2.jsonl"
