...
## Protocol clarification 11 — oracle build pin and CI boundary

On 2026-08-14, GitHub Actions run `31817307372` failed while recreating the oracle for a
README-only pull request. The resolved runtime versions, lock, and 5,470-file package inventory
matched, but `mlx_lm-0.31.3.dist-info/WHEEL` had been generated by setuptools 84.0.0 instead of
83.0.0. The pinned `mlx-lm` revision uses a legacy `setup.py`, so its isolated build backend was
not constrained by the runtime dependency lock. The oracle project now constrains that build
dependency to setuptools 83.0.0 and records the constraint in `uv.lock`; a cold rebuild restores
the accepted package and `mlx-lm` tree identities. The reviewed oracle lock is now
`b59b9022be34f429962150b4aa6a8339b7d4250c6ec814294b5d0d47663c540e`; this supersedes
clarification 2's earlier lock digest while retaining its runtime and package-tree identities.

Ordinary hosted pull-request CI no longer creates the 305 MB oracle environment. It retains the
pure Python oracle negative controls, model-free Rust and native suites, corpus/schema checks,
and append-only policy. A documentation-only change runs only diff hygiene plus the append-only
positive and negative controls. The protected self-hosted release and M1 measurement gates still
recreate the exact oracle, enforce the strict runtime and package identities, and run the live
startup controls before any real-model work.

This supersedes clarification 10 only where it says the hosted tier-1 job binds the installed
interpreter and package trees. It does not relax the measurement environment, model identity,
release gate, or any evidence-acceptance rule.
