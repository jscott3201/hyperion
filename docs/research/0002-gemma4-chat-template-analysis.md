...
important correctness fixes: JSON `null`, fail-loud rejection of stringified tool arguments,
controlled reasoning preservation, balanced continuation turns, and safer optional-input
handling. The remaining differences make the canonical template preferable for Hyperion's
transcript policy, which follows [Google's prompt-format guidance][google-prompt]: strip
thinking across ordinary turns while retaining the live function-calling turn.
...
| Thought retention | Keeps current-turn reasoning; historical reasoning is restored only when `preserve_thinking=true` and that message has tool calls | Requires tool calls even for current-turn reasoning and preserves historical tool reasoning by default | Use canonical behavior: strip ordinary prior-turn thoughts and retain a live tool-call chain. |
| Post-tool generation | With thinking enabled, resumes with `<\|channel>thought\n` after a terminal tool response | Does not add that continuation cue | Freeze the canonical suffix in a golden fixture. |
...
## M3 golden and negative fixtures
